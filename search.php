...
<head>
  ...
  <title>Gaming for Life Store - Search</title>
...
<body>
  ...
  <div><h1 style="text-align:center;" class="featurette-heading">Store: Search</h1></div>
  ...
  if(isset($_GET['name'])){
  $query="SELECT * FROM proizvodi WHERE ime='".($_GET['name'])."'";
  if (!$q=$mysqli->query($query)){
    echo "<p>There was an error. Please try again later</p>";
    exit();
  }
  if ($q->num_rows==0){
    echo "There are no games with such name";
  } else {
